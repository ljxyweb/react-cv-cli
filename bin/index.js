  ...
  .action(name => {
    if (!fs.existsSync(name)) {
      inquirer
        .prompt([
          {
            ...
            choices: [
              {
                name: "react-component-webpack-tpl",
                value: "1"
              },
              {
                name: "react-component-rollup-tpl",
                value: "2"
              }
            ]
          },
        ...
        .then(answers => {
          if (answers.option !== "1" && answers.option !== "2") {
            // 错误提示:输入的模板编号不正确。
            console.log(symbols.error, chalk.red("输入的模板编号不正确"));
            return;
          }
          ...
          spinner.start();
          download("ljxyweb/" + tpl, name, { clone: true }, err => {
            if (err) {
              spinner.fail();
